...
Latest bounded acceptance: [Final Intelligence Closure](./FINAL_INTELLIGENCE_CLOSURE_2026.md).
That report supersedes older evidence logs for checks repeated in this pass.
RA-T004 is now PRODUCTION_VERIFIED; RA-T003/006/007/009 remain PARTIAL.
The faster comparison candidate was tested, rejected on factual quality, and
rolled back. Release E remains NO-GO.
...
| C — Source freshness | RA-T005, RA-T007 | PARTIAL — cross-host fixture verified; exact official low-quality extraction case still unverified live |
...
| RA-T004 | S1 | Stale public capability JSON could override exact semantic state | Existing readiness contract and vector safety (unchanged this pass) | Exact semantic truth retained; no ordinary vector deletion | Semantic truth/fallback/vector safety suite; ten live readiness probes | Earlier exact inventory 67/67; final suite 689 pass/2 skip | Restored backend dpl_3mkM7SSkRvFBjJzoi5eknC2B3wCt | 10/10 public readiness responses match DB; all ten correctly typed document lookups HTTP 200; two hybrid and eight lexical controls | PRODUCTION_VERIFIED | Reference/namespace presence alone never establishes semantic readiness; no backfill or vector deletion |
...
| RA-T007 | S2 | External authority and source usability must remain distinct | Existing authority/source architecture unchanged | Verified fetched-host identity and purpose gate | Full authority/SSRF suite; live controlled canonical fixture and purpose matrix | Suite green | Existing production code; see FINAL_INTELLIGENCE_CLOSURE_2026.md | Cross-host canonical rejected; generic research LIMITED, compliance/current NOT_USABLE; PDF chat USER_SOURCE; private addresses rejected | PARTIAL | Official low-quality candidate failed at fetch, not extraction; upload response display metadata remains null/external despite correct chat identity |
...
| RA-T009 | S2 | Monolithic JSON consumed token budget; repair timed out; excerpt differences can be mistaken for legislative changes | Candidate comparisonThemes/service/provider/tests at 3d65e20, reverted by 0fa57d7 | Candidate uses six themes/two parallel Gemini batches and per-theme verification; not retained in production | Candidate 692 pass/2 skip; restored code 689 pass/2 skip | Faster but factual acceptance not certified | Candidate dpl_FH6T3YH81Ej44ssxYPDkFDssNDwH tested then rolled back | Six AI results, all PARTIAL_EVIDENCE; normal median 7.489s; two immutable regenerations; DPDP inference still fails strict acceptance | PARTIAL | Candidate preserved for separate review; no third tuning round; previous production code restored |
