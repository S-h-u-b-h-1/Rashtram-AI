  ...
  const libraryPanel = (
    <section className="flex h-full min-h-0 flex-col bg-[#f8f6f1]">
      <div className="border-b border-[#8f1d2c]/10 px-4 py-4">
        <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#874047]">Rashtram library</p>
        <h2 className="mt-1 text-lg font-semibold text-[#29312d]">Policy references</h2>
        <p className="mt-2 text-xs leading-5 text-[#706a61]">References are matched to the problem, audience, and jurisdiction in your draft brief.</p>
        ...
        {filteredDocuments.map((document) => {
          ...
          return <button key={document.id} type="button" aria-pressed={selected} disabled={drafting} onClick={() => toggleDocument(document.id)} className={`w-full rounded-2xl border p-3 text-left transition ${selected ? "border-[#8f1d2c]/35 bg-[#fffaf0] shadow-sm" : "border-[#8f1d2c]/8 bg-white hover:border-[#8f1d2c]/20"}`}>
            <div className="flex items-start gap-2">
              ...
              <span className="min-w-0 flex-1"><span className="line-clamp-3 block text-xs font-semibold leading-5 text-[#29312d]">{document.title}</span><span className="mt-1 block text-xs uppercase tracking-[0.1em] text-[#706a61]">{document.type} · {document.ministry || document.authority || "India"}</span><span className="mt-1 block text-xs font-semibold text-[#34725b]">Ready to use</span></span>
            </div>
          </button>;
        })}
      </div>
      <div className="border-t border-[#8f1d2c]/10 px-4 py-3 text-xs text-[#706a61]">{selectedDocumentIds.length} catalogue references selected</div>
    </section>
  );
