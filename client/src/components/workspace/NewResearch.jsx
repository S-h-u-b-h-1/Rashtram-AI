...
export function NewResearch() {
  ...
  return <div className={`mx-auto max-w-[840px] ${searchedQuestion || showSources ? "pt-2" : "pt-[clamp(1rem,9vh,6rem)]"}`}>
    ...
    {searchedQuestion && (problemUnderstanding || researchPlan.length > 0) && <section className="mt-5 rounded-xl border border-[#8f1d2c]/12 bg-[#fffaf0] p-4" aria-label="Research understanding and recommendations">
      {problemUnderstanding && <div><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#874047]">Understanding your research</p><h3 className="mt-1 font-serif text-xl text-[#8f1d2c]">{problemUnderstanding.statement}</h3><p className="mt-2 text-xs leading-5 text-[#706a61]">Goal: {problemUnderstanding.goal} · Jurisdiction: {problemUnderstanding.jurisdiction} · Authority: {problemUnderstanding.regulator}</p></div>}
      {researchPlan.length > 0 && <div className="mt-4"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[#874047]">Topics that matter</p><div className="mt-2 flex flex-wrap gap-2">{researchPlan.slice(0, 6).map((item) => <span key={item.area} className="rounded-full border border-[#8f1d2c]/10 bg-white px-2.5 py-1.5 text-xs text-[#625d55]">{item.area}</span>)}</div></div>}
