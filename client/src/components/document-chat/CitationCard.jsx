...
export function CitationCard({ source, index }) {
  ...
  return <details className="max-w-full rounded-lg border border-[#8f1d2c]/15 bg-white text-xs">
    ...
    <div className="max-w-[520px] border-t border-[#8f1d2c]/10 p-3">
      {source.authorityClass === "USER_SOURCE" && <p className="mb-1 text-xs font-semibold text-[#706a61]">User-uploaded source</p>}
      <p className="text-xs leading-5 text-[#706a61]">{[source.authority || source.ministry, formatDate(source.publicationDate || source.date, ''), source.pageStart ? `Page ${source.pageStart}${source.pageEnd && source.pageEnd !== source.pageStart ? `–${source.pageEnd}` : ''}` : null, sectionLabel, String(source.languageCode || '').startsWith('hi') ? 'Original Hindi' : null].filter(Boolean).join(' · ')}</p>
