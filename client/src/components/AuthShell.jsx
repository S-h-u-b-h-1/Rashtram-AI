...
export function AuthShell({ eyebrow, title, description, children }) {
  return (
    <main className="min-h-screen bg-[#f4efe5] lg:grid lg:grid-cols-[0.9fr_1.1fr]">
      <section className="relative hidden min-h-screen overflow-hidden bg-[#c30000] p-10 text-white lg:flex lg:flex-col xl:p-14">
        ...
        <div className="absolute -right-24 bottom-0 h-80 w-80 rounded-full bg-[#d97745]/20 blur-3xl" />

        <BrandMark inverse className="relative z-10" />

        <div className="relative z-10 my-auto max-w-xl">
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#efb36f]">
            {eyebrow}
          </p>
          ...
          <div className="mt-10 grid gap-3">
            {assurances.map((item) => (
              ...
              >
                <CheckCircle2 className="h-4 w-4 text-[#efb36f]" />
                {item}
              </div>
            ))}
          </div>
      ...
      <section className="relative flex min-h-screen items-center justify-center px-5 py-8 sm:px-8 lg:px-12">
        <Link
          href="/"
          className="absolute left-5 top-5 inline-flex items-center gap-2 rounded-full px-3 py-2 text-sm text-[#5f5a52] transition hover:bg-white/60 hover:text-[#c30000] sm:left-8 sm:top-7"
        >
        ...
        <div className="w-full max-w-[470px]">
          ...
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#ad4a36]">
            {eyebrow}
          </p>
          <h1 className="mt-3 font-serif text-4xl tracking-[-0.03em] text-[#c30000] sm:text-5xl">
            {title}
          </h1>
