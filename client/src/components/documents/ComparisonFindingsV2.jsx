...
export function ComparisonFindingsV2({ result }) {
  ...
  return <div className="min-w-0 space-y-5 [overflow-wrap:anywhere]">
    <section className="surface-card p-5 sm:p-6">
      <p className="text-sm font-semibold text-[#8f1d2c]">{findings.length} supported findings</p>
    ...
    {findings.map(f => {
      ...
      return <article key={f.id} className="surface-card min-w-0 p-5 sm:p-6">
        ...
        <details className="mt-4 border-t border-[#8f1d2c]/10">
          ...
          {[f.sourceA, f.sourceB].map((side, i) => <div key={i} className="mb-4">
            <p className="text-xs font-bold text-[#706a61]">{names[i]} · {side.section || 'Source passage'}</p>
