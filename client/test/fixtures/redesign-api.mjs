...
comparisons[0].result = { ...comparisons[0].result, generationMode: 'gemini', comparisonSchemaVersion: 'comparison-quality-v3', quality: { outputValidation: { valid: true, status: 'SUCCESS' } }, differences: [{ topic: 'Instrument type', documentA: 'Published Act', documentB: 'Draft Bill', whyItMatters: 'Verify legal force independently.', citations: ['D101-P1'] }], whatChanged: [{ content: 'Illustrative change explanation.', citations: ['D101-P1'] }], practicalImplications: [{ content: 'Review applicability.', citations: ['D101-P1'] }], limitations: ['QA fixture only.'] };
comparisons.push({ ...comparisons[0], id: '2', result: { ...comparisons[0].result, quality: { outputValidation: { valid: true, status: 'PARTIAL_EVIDENCE' } } } });
comparisons.push({ ...comparisons[0], id: '3', result: { ...comparisons[0].result, generationMode: 'evidence_abstention', executiveSummary: 'Insufficient evidence in this synthetic fixture.', quality: { outputValidation: { valid: false, status: 'INSUFFICIENT_EVIDENCE' } } } });
...
http.createServer(async (req, res) => {
  ...
  if (path.endsWith('/pdf')) { const { createComparisonPdf } = await import('../../../server/document/comparisonPdfPresentation.js'); const comparison = comparisons.find(item => item.id === path.split('/')[3]) || comparisons[0]; const pdf = await createComparisonPdf(comparison); res.writeHead(200, { ...headers, 'Content-Type': 'application/pdf' }); res.end(pdf); return; }
  if (path === '/product-intelligence/compliance') return json(res, { problemUnderstanding: { statement: 'You are researching digital-service requirements in India.', jurisdiction: 'India', regulator: 'Relevant authority', goal: 'Understand applicable requirements', stakeholders: ['Customers'], timeframe: 'Verify currentness.' }, researchPlan: ['Institutional model', 'Customer protection', 'Data handling', 'Grievance redress'].map((area, i) => ({ area, order: i + 1, priority: i ? 'important' : 'essential', rationale: 'Inspect applicable sources.' })), discoveryGroups: { essential: [{ ...documents[0], confidence: 'high', authorityClass: 'PRIMARY_OFFICIAL', authorityLabel: 'Primary official source', priority: 'essential', whyThisMatters: 'Investigate data roles.', focusAreas: ['Consent', 'Security'] }], important: [{ ...documents[1], confidence: 'high', priority: 'important', authorityLabel: 'Draft proposal', whyThisMatters: 'Understand the earlier proposal.', focusAreas: ['Scope'] }], background: [] }, recommendations: [{ ...documents[0], authorityClass: 'PRIMARY_OFFICIAL' }], lowerConfidenceRecommendations: [{ ...documents[2], confidence: 'low', priority: 'background' }], missingEvidence: ['Verify commencement'], disclaimer: 'Synthetic UX fixture only.' });
  if (path.startsWith('/documents/') && path.endsWith('/graph')) return json(res, { graph: { currentDocument: documents[0], nodes: [], edges: [], relationships: [] } });
  if (path.startsWith('/product-intelligence/amendments/')) return json(res, { status: 'no verified amendment chain', affectedSections: [] });
  if (path === '/product-intelligence/watchlists/refresh') return json(res, { created: 0 });
  ...
  if (/^\/documents\/compare(?:\/\d+(?:\/regenerate)?)?$/.test(path)) { const comparison = comparisons.find(item => item.id === path.split('/')[3]) || comparisons[0]; return json(res, { comparison, comparisonId: comparison.id, ...comparison.result }); }
