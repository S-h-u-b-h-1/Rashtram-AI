...
const { buildFilters } = require('../document/DocumentRepository');
...
test('catalogue discovery is broad by default and readiness filtering stays explicit', () => {
  const broad = buildFilters({ search: 'digital lending' });
  const readyOnly = buildFilters({ search: 'digital lending', researchReady: true });
  assert.doesNotMatch(broad.where, /readiness_document\.research_ready/);
  assert.match(readyOnly.where, /readiness_document\.research_ready/);
});
